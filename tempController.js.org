...
/*
	if( dir == 1 && pwm > 500 ){
		//console.log(getTimeDate());
		//console.log('Overheat warnning > 500  ['+pwm+']');
		//console.log('set CurretTemp['+currentTemp+'] as TargetTemp['+targetTemp+']');
		pwm = 300;
		ctr.setTarget(currentTemp); // set CurrentTemp to TargetTemp
		} else {
...
//  k_i: 0.01,
//  k_d: 0.01,
//  dt: 0.5, // dt: interval of time (in seconds) between two measures. 
//  i_max: 1000
//  If not provided, it will be automatically calculated.
});

//ctr = new Controller(0.25, 0.01, 0.0025, 0.5, 1000);
...
setInterval(function(){
		...
		if( unFreezeState === false ){
                if( isNaN(currentTemp) === false ){
  			input  = ctr.update(currentTemp);
			//console.log('ctr.update='+input.toFixed(1));
...
// check condensation
	...
	/*
		if( uncondense ){
			console.log('unCondensation process');
