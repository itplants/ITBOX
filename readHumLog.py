...
#print(re.search('currentHum=',lines1))
...
for i in range(start*9,len(lines1)):
    ...
    if line.find('currentHum') >= 0:
	hum=np.append(hum,line[:])
...
for i in range(start,len(hum)-1):
    print date[i+1]+','+hum[i].split(' ')[1]+','+hum[i].split(' ')[3]+','+pwm[i].split(' ')[1]
'''
#+' '+bme280[i]+' '+lux[i]+' '+cons[i]+' '+ds18b20[i]
