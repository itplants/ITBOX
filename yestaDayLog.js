...
var DEBUG=false; // if DEBUG is true then testdata folder is used.
...
date=fileName;
var csvFileName='';
if( !DEBUG ) csvFileName='./data/tempController'+fileName+'.csv';
else csvFileName='./testdata/tempController'+fileName+'.csv';
...
var fs = require('fs');
var ls='';
if( !DEBUG ) ls=fs.readdirSync('./data/');
else ls=fs.readdirSync('./testdata/');

console.log('ls='+ls);
var todayfiles=[];
for(i=0;i<ls.length;i++){
if(ls[i].indexOf(date)>=0 && ls[i].indexOf('.log')>=0){
  if(!DEBUG)
    todayfiles.push('./data/'+ls[i]);
  else 
    todayfiles.push('./testdata/'+ls[i]);
  }
}
